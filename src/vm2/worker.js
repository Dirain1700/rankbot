...
const { getPrototypeOf, getOwnPropertyDescriptor, getOwnPropertyDescriptors, defineProperty } = Object;
...
const typedArrayProtoDescriptors = getOwnPropertyDescriptors(typedArrayPrototype);
...
const errorToString = (err) => {
    if (typeof err === "object" && err instanceof Error) return Error.prototype.toString.call(err);
...
const run = async (code) => {
    ...
    const outStream = defineProperty(new Writable(), "write", {
        value(chunk, encoding, callback) {
            ...
            else if (isUint8Array(chunk)) chunk = makeBufferFrom(arrayBuffer(chunk), offset(chunk), length(chunk));
            //prettier-ignore
            else throw new TypeError("The \"chunk\" argument must be of type string or an instance of Buffer or Uint8Array");

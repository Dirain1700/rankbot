module.exports = (message) => {
    ...
    if (!codeBlockRegex.test(content)) return message.reply("Please send code!").catch(console.error);
    const { code } = content.match(codeBlockRegex)?.groups ?? {};

    pool.exec("run", [code]) //prettier-ignore
        .timeout(5000)
        .then(([consoleOutput, result]) => message.reply(toMessageOptions(consoleOutput, result)))

module.exports = async (client, interaction) => {
    ...
    await interaction.reply({
        content: `${time(new Date(), "T")} ${targetUser.tag} was force-banned from ${interaction.guild.name} for ${day}days by ${
            interaction.user.tag
        }.(${reasons})`,
        ephemeral: false,
    });
    await targetUser.send(
        `${time(new Date(), "T")} You (${targetUser.tag}) were banned from ${interaction.guild.name} for ${day}days by ${interaction.user.tag}.(${reasons})`
    );

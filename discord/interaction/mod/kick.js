module.exports = async (client, interaction) => {
  ...
  interaction.reply({ content: `${time(new Date(), "T")} ${targetMember.user.tag} was kicked from ${interaction.guild.name} by ${interaction.user.tag}.(${reasons})`, ephemeral: false });
  targetMember.user.send(`${time(new Date(), "T")} You (${targetMember.user.tag}) were kicked from ${interaction.guild.name} by ${interaction.user.tag}.(${reasons})`);
